...
// 이유 : HTML은 연산X , 저장 X 등등 없으므로 텍스트 그자체
// 새로운값/데이터 을 입력받아서 데이터 가공(JS) 하기위해서 JS 사용한다.
// 순서도(알고리즘) , 어떠한 작업의 순서/절차
    // html 과 css 는 절차X , JS코드는 위에서  아래로 실행
    // 1. 1명의 사원이름을 입력받는다.                   prompt() , 반환값 : 입력받은값
    // 2. 입력받은 사월이름을 변수에 저장한다.            let 사원1 = prompt()              
    // 3. 1명의 사원이름을 입력받는다.                   prompt() , 반환값 : 입력받은값
    // 4. 입력받은 사원이름을 변수에 저장한다.            let 사원2 = prompt()
    // 5. HTML형식의 문자로 출력할 변수와 구성 한다.      
    // 6. 구성된 HTML를 HTML문서에 출력한다.
    // - 변수를 사용하는 이유 : 다음 코드에서 사용(출력 목적) 할려고
let 사원1 = prompt('사원이름을 입력해주세요.') // [1][2] prompt() 함수를 사용하여 입력받은 값을 변수에 대입했다.
let 사원2 = prompt('사원이름을 입력해주세요.') // [3][4] prompt() 함수를 사용하여 입력받은 값을 변수에 대입했다.
                                            // [5] 
let tableHTML = `<table>                    
                <tr> <th> 번호 </th> <th> 이름 </th> </tr>
                <tr> <td> 1 </td> <td> ${ 사원1 } </td> </tr>
                <tr> <td> 2 </td> <td> ${ 사원2 } </td> </tr> 
                </table>`
